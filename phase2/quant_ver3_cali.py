...
# - V6(현재): sel_omniquant에 추가된 selective + omniquant base 기반 알고리즘을 calibration 방식에 맞게 변경
...
class QuotRemLinear(nn.Module):
    ...
    def __init__(
        ...
        selective_base_threshold=1.0,
        selective_int_bits=4,
        residual_clip_alpha=0.0,
        debug_name="",
    ):
        super().__init__()
        ...
        self.selective_base_threshold = selective_base_threshold
        self.selective_int_bits = selective_int_bits
        ...
        # scale_r은 이전 실패(v1)처럼 static으로 고정하지 않고 runtime으로 둔다.
    ...
    def finalize_calib(self, base_quantile: float = 0.5):
        ...
        # nearest power-of-2 base: selective+OmniQuant runtime path와 같은 base policy.
        ...
        base       = torch.where(dist_floor <= dist_ceil, base_floor, base_ceil).clamp(2**(-7), 128.0)
    ...
    def forward(self, x: torch.Tensor):
        ...
        # ---- 추론: 고정 base/sign + selective fallback + runtime R scale ----
        ...
        max_abs   = xg_base.abs().amax(dim=-1, keepdim=True).clamp_min(eps)    # fallback INT scale은 runtime 입력 기준
        if self.q_bits == 1:
            ...
            q_scaled_base = q * fixed_sign_flag * fixed_base
        else:
            q_max_val = (2 ** (self.q_bits - 1)) - 1
            q = torch.round(xg_base / fixed_base).clamp(-q_max_val, q_max_val)
            r = xg_base - fixed_base * q
            q_scaled_base = q * fixed_base

        # selective routing: fixed_base 기반 정적 판단 → token별 adaptive routing 불가 (mode mismatch)
        # 동적 버전과 동일하게 runtime max_abs 에서 nearest power-of-2 를 추정해 routing 결정.
        # base/sign_flag 는 Q 계산에 calibrated fixed 값을 계속 사용하되,
        # "이 group 이 QR 을 쓸 것인가 INT4 를 쓸 것인가" 판단만 token-adaptive 로 복원.
        log2_rt    = torch.log2(max_abs)
        base_rt_fl = torch.pow(2.0, torch.floor(log2_rt))
        base_rt_ce = torch.pow(2.0, torch.ceil(log2_rt))
        base_rt    = torch.where(
            (max_abs - base_rt_fl).abs() <= (base_rt_ce - max_abs).abs(),
            base_rt_fl, base_rt_ce
        ).clamp(2**(-4), 128.0)                                                # [..., G_base, 1]
        selective_mask = base_rt < self.selective_base_threshold
        ...
        if self.residual_clip_alpha > 0:
            # 실험용 선택지: 기본값 0에서는 완전히 비활성화된다.
            base_flat = fixed_base.expand_as(xg_base).reshape(orig_shape)
            ...
            max_r = torch.minimum(max_r, clip_bound_grp).clamp_min(eps)
            r_input = torch.maximum(torch.minimum(r_grp, clip_bound_elem), -clip_bound_elem)
...
def parse_module_float_map(s):
    ...
    for item in s.split(","):
        ...
        key = key.strip()
        if key not in {"self_attn.q_proj","self_attn.k_proj","self_attn.v_proj",
                       "self_attn.out_proj","fc1","fc2"}:
            raise ValueError(f"Invalid module threshold key: {key}")
...
def main():
    ...
    parser.add_argument("--output_dir",            type=str, default="quotrem_ppl_results_v3_sel_omniweight_cali")
    ...
    parser.add_argument("--selective_base_threshold", type=float, default=8.0,
                        help="calibrated base가 이 값보다 작은 group은 QR 대신 INT fallback 적용")
    ...
    parser.add_argument("--residual_clip_alpha", type=float, default=0.0,
                        help=">0이면 R quant scale을 alpha*base로 cap하고 R을 clip. 0이면 비활성화")
    ...
    lines = [
        ...
        "",
        "[OmniQuant Weight]",
        "source             : OmniQuant checkpoint loaded by --model_id",
        "enable_weight_quant: False  # forced off to avoid double quantization",
        f"ignored_cli_enable_weight_quant: {args.enable_weight_quant}",
        ...
        f"selective_base_threshold : {args.selective_base_threshold}",
        f"module_selective_base_thresholds : {module_selective_base_thresholds}",
        f"selective_int_bits       : {args.selective_int_bits}",
        f"residual_clip_alpha      : {args.residual_clip_alpha}",
        "",
    ]
    ...
    replaced = replace_modules(
        ...
        use_weight_quant, args.weight_bits, args.weight_quant_mode,
        ...
        module_selective_base_thresholds,
        args.selective_int_bits,
        args.residual_clip_alpha,
    )
