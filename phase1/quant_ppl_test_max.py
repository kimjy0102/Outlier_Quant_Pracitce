...
def fake_quant_symmetric(
    ...
    group_size: int = None,
    eps: float = 1e-8,
    return_group_absmax: bool = False,
):
    """
    mode:
        - tensor
    ...
    if mode == "tensor":
        ...
        dq = (q * scale).to(x.dtype)
        if return_group_absmax:
            return dq, None
        return dq
        ...
        max_abs = xg.abs().amax(dim=-1, keepdim=True)   # [..., num_groups, 1]
        scale = (max_abs / qmax).clamp_min(eps)

        q = torch.round(xg / scale).clamp(qmin, qmax)
        dq = q * scale
        dq = dq.reshape_as(x_fp).to(x.dtype)

        if return_group_absmax:
            group_absmax = max_abs.squeeze(-1)          # [..., num_groups]
            return dq, group_absmax
        return dq
...
class ExperimentLinear(nn.Module):
    ...
    def __init__(
        ...
        debug_name: str = "",
        collect_group_absmax: bool = False,
    ):
        super().__init__()
        ...
        self.collect_group_absmax = collect_group_absmax
        # weight group absmax stats
        self.weight_group_absmax_sum = None
        self.weight_group_absmax_max = None
        self.weight_group_absmax_count = 0
        # activation group absmax stats
        self.act_group_absmax_sum = None
        self.act_group_absmax_max = None
        self.act_group_absmax_count = 0
        ...
        # fake-quant cache
        self.register_buffer("weight_q_cache", torch.empty(0), persistent=False)
    ...
    # abs max
    # ------------------------------------------------------
    def _update_group_absmax_stats(self, group_absmax: torch.Tensor, target: str):
        """
        group_absmax shape:
          - weight group: [out_features, num_groups]
          - act group   : [B, S, num_groups] or [B, num_groups]
        """
        if group_absmax is None:
            return

        g = group_absmax.detach().float()
        g = g.reshape(-1, g.shape[-1]).cpu()   # [N, num_groups]

        sum_vec = g.sum(dim=0, dtype=torch.float64)
        max_vec = g.max(dim=0).values.to(torch.float32)
        count = g.shape[0]

        if target == "weight":
            if self.weight_group_absmax_sum is None:
                self.weight_group_absmax_sum = sum_vec
                self.weight_group_absmax_max = max_vec
                self.weight_group_absmax_count = count
            else:
                self.weight_group_absmax_sum += sum_vec
                self.weight_group_absmax_max = torch.maximum(
                    self.weight_group_absmax_max, max_vec
                )
                self.weight_group_absmax_count += count

        elif target == "act":
            if self.act_group_absmax_sum is None:
                self.act_group_absmax_sum = sum_vec
                self.act_group_absmax_max = max_vec
                self.act_group_absmax_count = count
            else:
                self.act_group_absmax_sum += sum_vec
                self.act_group_absmax_max = torch.maximum(
                    self.act_group_absmax_max, max_vec
                )
                self.act_group_absmax_count += count
        else:
    ...
    def reset_act_group_absmax_stats(self):
        self.act_group_absmax_sum = None
        self.act_group_absmax_max = None
        self.act_group_absmax_count = 0
    # -----------------------------------------------------
    ...
    def refresh_weight_cache(self):
        if self.quant_impl == "fake":
            if self.enable_weight_quant:
                if self.collect_group_absmax and self.weight_quant_mode == "group":
                    w_q, weight_group_absmax = fake_quant_symmetric(
                        self.weight,
                    ...
                    self.weight_q_cache = w_q.detach()
                    self._update_group_absmax_stats(weight_group_absmax, target="weight")
                else:
                ...
                self.weight_q_cache = torch.empty(
                    0, device=self.weight.device, dtype=self.weight.dtype
                )

            self.weight_q_int_cache = torch.empty(
                0, device=self.weight.device, dtype=torch.int32
            )
            self.weight_scale_cache = torch.empty(
                0, device=self.weight.device, dtype=self.weight.dtype
            )
        # hw like quantization 
            ...
            if self.enable_weight_quant:
                q_int, scale = quantize_weight_symmetric_int_chunked(
                    ...
                    cache_device=self.weight_cache_device,   # 일단 cpu 추천
                )
                self.weight_q_int_cache = q_int.detach()
                self.weight_scale_cache = scale.detach()
            else:
                self.weight_q_int_cache = torch.empty(
                    0, device="cpu", dtype=torch.int8
                )
                self.weight_scale_cache = torch.empty(
                    0, device="cpu", dtype=self.weight.dtype
                )
    ...
    def _apply_activation_quant_fake(self, x):
        ...
        if self.collect_group_absmax and self.act_quant_mode == "group":
            x_q, act_group_absmax = fake_quant_symmetric(
                x,
            ...
            self._update_group_absmax_stats(act_group_absmax, target="act")
            return x_q
    ...
    for layer_idx in layer_indices:
        ...
        for module_name in module_names:
            ...
            new_module = ExperimentLinear(
                ...
                collect_group_absmax=collect_group_absmax,
...
# absmax text save
# ========================================================
def dump_group_absmax_stats(model, output_dir, topk=10):
    lines = []
    lines.append("[Group AbsMax Stats]")
    lines.append("")

    found_any = False

    for module in model.modules():
        if not isinstance(module, ExperimentLinear):
            continue

        name = module.debug_name if len(module.debug_name) > 0 else "<unnamed>"

        # weight
        if module.weight_group_absmax_count > 0:
            found_any = True
            mean_vec = (module.weight_group_absmax_sum / max(module.weight_group_absmax_count, 1)).to(torch.float32)
            max_vec = module.weight_group_absmax_max

            k = min(topk, max_vec.numel())
            top_vals, top_idx = torch.topk(max_vec, k)

            lines.append(f"{name} [WEIGHT]")
            lines.append(f"  group_size        : {module.weight_group_size}")
            lines.append(f"  num_groups        : {max_vec.numel()}")
            lines.append(f"  num_vectors_seen  : {module.weight_group_absmax_count}")
            lines.append(f"  overall_mean(max) : {mean_vec.mean().item():.8f}")
            lines.append(f"  overall_max(max)  : {max_vec.max().item():.8f}")
            lines.append(f"  top{k}_group_idx  : {top_idx.tolist()}")
            lines.append(f"  top{k}_group_max  : {[float(v) for v in top_vals]}")
            lines.append("")

        # activation
        if module.act_group_absmax_count > 0:
            found_any = True
            mean_vec = (module.act_group_absmax_sum / max(module.act_group_absmax_count, 1)).to(torch.float32)
            max_vec = module.act_group_absmax_max

            k = min(topk, max_vec.numel())
            top_vals, top_idx = torch.topk(max_vec, k)

            lines.append(f"{name} [ACTIVATION]")
            lines.append(f"  group_size        : {module.act_group_size}")
            lines.append(f"  num_groups        : {max_vec.numel()}")
            lines.append(f"  num_vectors_seen  : {module.act_group_absmax_count}")
            lines.append(f"  overall_mean(max) : {mean_vec.mean().item():.8f}")
            lines.append(f"  overall_max(max)  : {max_vec.max().item():.8f}")
            lines.append(f"  top{k}_group_idx  : {top_idx.tolist()}")
            lines.append(f"  top{k}_group_max  : {[float(v) for v in top_vals]}")
    ...
    if not found_any:
        lines.append("No group absmax stats were collected.")
        lines.append("Check that:")
        lines.append("  - --collect_group_absmax is enabled")
        lines.append("  - quant mode is group")
        lines.append("  - forward actually ran")
        lines.append("")

    save_txt(lines, Path(output_dir) / "group_absmax_summary.txt")
    return lines
...
def main():
    ...
    # absmax
    parser.add_argument("--collect_group_absmax", action="store_true")
    parser.add_argument("--group_absmax_topk", type=int, default=10)
    ...
    lines.append(f"collect_group_absmax: {args.collect_group_absmax}")
    lines.append(f"group_absmax_topk  : {args.group_absmax_topk}")
    lines.append("")
    ...
    replaced_names = replace_modules_with_experiment_linear(
        ...
        collect_group_absmax=args.collect_group_absmax,
    )
    ...
    # probe 입력이 activation group absmax 통계에 섞이지 않게 reset
    if args.collect_group_absmax:
        for m in model.modules():
            if isinstance(m, ExperimentLinear):
                m.reset_act_group_absmax_stats()
    # -----------------------------------------------------
    # Modified PPL
    # -----------------------------------------------------
    ...
    print("\n".join(lines))
    if args.collect_group_absmax:
        group_lines = dump_group_absmax_stats(
            model=model,
            output_dir=args.output_dir,
            topk=args.group_absmax_topk,
        )
